...
export const advisory: Strategy = async (req) => {
  const t0 = Date.now();
  try {
    await sql.begin(async (tx) => {
      await tx`SELECT pg_advisory_xact_lock(${req.roomId})`;
      ...
      await tx`
        ...
        VALUES (${req.roomId}, ${req.doctorId}, ${req.startsAt}::timestamptz, ${req.endsAt}::timestamptz, 'advisory')
      `;
    });
