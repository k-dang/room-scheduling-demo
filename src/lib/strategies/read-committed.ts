...
export const readCommitted: Strategy = async (req) => {
  const t0 = Date.now();
  try {
    await sql.begin(async (tx) => {
      ...
      // App work between the check and the write — the race window even inside a transaction.
      await new Promise((r) => setTimeout(r, 50));
      await tx`
        INSERT INTO room_scheduling.bookings (room_id, doctor_id, starts_at, ends_at, strategy)
        VALUES (${req.roomId}, ${req.doctorId}, ${req.startsAt}::timestamptz, ${req.endsAt}::timestamptz, 'read-committed')
      `;
    });
